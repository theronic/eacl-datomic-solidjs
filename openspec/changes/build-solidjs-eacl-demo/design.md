...
Top-level signals will hold the active subject, permission, selected resource, page size, cache mode, theme, and latest mutation revision. Resource-group components own cursor stacks and `createResource` instances whose sources are stable tuples of only the inputs they actually use. Count resources exclude page cursors, start with an independent 50,000-result demand limit, and retain a group-local limit that doubles only when the user clicks a truncated total. A subject, permission, cache-mode, or mutation-revision change resets that demand to 50,000 before issuing the replacement count. Nested relationship resources exist only while expanded, and selected-resource detail resources exist only when a resource is selected.

Changing subject, permission, page size, schema revision, or cache mode resets affected cursor stacks before refetch. Changing only a page cursor leaves the independent count resource intact. The stats line gives page range/timing and bounded-or-exact count/timing in separate Solid-owned DOM fragments so a page refresh cannot replace or flash the stable count fragment. A truncated count is a keyboard-operable button such as `50k+`; clicking it doubles the request limit for that group, preserves the prior value while the replacement request is pending, and becomes non-clickable when EACL reports `:truncated? false`. Request helpers use `AbortController` and request identities so superseded responses cannot overwrite current state. `<For>` keys use stable type/id or permission identities, `<Show>` isolates conditional branches, and `createMemo` handles pure projections. This is preferred to a global mutable store or an additional server-state library because native Solid resources already provide the needed dependency tracking with less machinery.
...
The server will call EACL's authenticated pagination and bounded count operations directly, never enumerate a complete result set to paginate it in the client, and never serialize Datomic entities. `POST /api/eacl/count-resources` validates a positive `countLimit` and passes it to EACL as `:count-limit`. Each expanded resource group performs one page query and one independent count query initially bounded at 50,000; the UI renders the total once alongside the page range and labels both timings without duplicating the count. Further count work requires an explicit click and doubles only that group's bound until EACL proves exhaustion. Relationship children load only when expanded; metadata that changes only with schema revision can be memoized on the server. Blocking Datomic/EACL work runs off the client and is instrumented per operation.
