...
(defn- handle-health
  ...
  (contracts/success
   system request
   {:status :ready
    :datomic {:basis (d/basis-t (d/db (:conn system)))}}))
...
(defn- handle-bootstrap
  ...
  (let [db (d/db (:conn system))]
    (contracts/success
     system request
     {:status (if (= :seeding (:status @(:!seed-progress system)))
                :seeding
                :ready)
      :seed @(:!seed-progress system)
      :totals (data/totals db)
...
(defn- handle-subjects
  ...
  (let [offset (contracts/query-long request :offset 0)
    ...
    (contracts/success
     system request (data/known-subjects (d/db (:conn system)) offset limit))))
...
(defn- handle-schema-read
  [system request]
  (contracts/success system request (data/schema-info (d/db (:conn system)))))
...
(defn- handle-cache-read
  ...
  (contracts/success
   system request
   {:provider (datomic-eacl/cache-stats (:acl system))
...
(defn- handle-cache-evict
  ...
  (locking (:evict-lock system)
    (datomic-eacl/expire-cache! (:acl system))
...
(defn- handle-seed-write
  ...
  (let [body (contracts/read-json request (get-in system [:config :max-body-bytes]))
    ...
    (let [servers-before (data/count-servers (d/db (:conn system)))
           ...
           (call [_]
             (try
               ...
               (catch Throwable throwable
                 (log/error throwable "Background seed job failed")
                 nil)))))
